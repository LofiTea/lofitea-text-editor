...
/*** defines ***/
#define LOFITEA_VERSION "1.0.0"
#define LOFITEA_TAB_STOP 8
#define LOFITEA_QUIT_TIMES 3
...
int editor_row_cx_to_rx(erow *row, int cx) {
    ...
    for (int j = 0; j < cx; j++) {
        if (row->chars[j] == '\t')
        rx += (LOFITEA_TAB_STOP - 1) - (rx % LOFITEA_TAB_STOP);
        rx++;
    }
    return rx;
}
...
int editor_row_rx_to_cx(erow *row, int rx) {
    ...
    for (cx = 0; cx < row->size; cx++) {
        if (row->chars[cx] == '\t') {
            cur_rx += (LOFITEA_TAB_STOP - 1) - (cur_rx % LOFITEA_TAB_STOP);
        }
        cur_rx++;
...
void editor_update_row(erow *row) {
    ...
    free(row->render);
    row->render = malloc(row->size + tabs*(LOFITEA_TAB_STOP - 1) + 1);

    int idx = 0;
    for (int j = 0; j < row->size; j++) {
        if (row->chars[j] == '\t') {
            row->render[idx++] = ' ';
            while (idx % LOFITEA_TAB_STOP != 0) {
                row->render[idx++] = ' ';
            }
        } else {
    ...
    for (int y = 0; y < E.screen_rows; y++) {
        ...
        if (filerow >= E.num_rows) {
            if ((E.num_rows == 0) && (y == E.screen_rows / 3)) {
                ...
                "LofiTea Editor -- Version %s", LOFITEA_VERSION);
...
void editor_process_keypress() {
    static int quit_times = LOFITEA_QUIT_TIMES;
    int c = editor_read_key();
        ...
        case PAGE_DOWN:
            {
                ...
                while (times--)
                editor_process_keypress(c == PAGE_UP ? ARROW_UP : ARROW_DOWN);
            }
            break;
        case ARROW_UP:
        case ARROW_DOWN:
...
void init_editor() {
    ...
    if (get_window_size(&E.screen_rows, &E.screen_cols) == -1) die("getWindowSize");
    E.screen_rows -= 2;
}
